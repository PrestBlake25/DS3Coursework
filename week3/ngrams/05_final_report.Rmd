...
author: "Preston Blake (Lehman College)"
date: '`r Sys.time()`'
output:
  html_document:
    toc: yes
...
```{r load-counts}
file1 <- here('week3/ngrams/data', 'total_counts.csv')
file2 <- here('week3/ngrams/data', 'year_counts.tsv')

year_counts <- read_tsv(file2, col_names = c('term', 'year','volume', 'book_count'))
total_counts <- read_csv(file1, col_names = c('year', 'total_volume', 'page_count', 'book_count'))
...
year_total_counts <- inner_join(total_counts, year_counts, by = "year")
```
...
year_total_counts <- year_total_counts %>% mutate(volume_perc = volume/total_volume)

filtered_years <- filter(year_total_counts, term %in% c(1883, 1910, 1950))

ggplot(data = filtered_years, aes(x = year, y = volume_perc, group = term ,color = term)) + 
geom_line() + xlim(1850, 2000) + labs(x = "Year", y = "Frequency") + scale_y_continuous(labels = percent)

```

## Your written answer

The version 2 of the NGram data does look similar to the results presented in the version 1 data.
...
Add your screenshot for Part C below this line using the `![week3/ngrams/NGramsBooks.png](figure_filename.png)` syntax and comment on similarities / differences.

![NGramsPlot](week3/ngrams/NGramsBooks.png)
...
ggplot(data = filtered_years, aes(x = year, y = volume, group = term ,color = term)) + 
geom_line() + xlim(1850, 2012) + labs(x = "Year", y = "Raw Mentions") + scale_y_continuous(labels = comma)
...
```{r plot-totals}
filtered_years$term <- as.factor(filtered_years$term)
ggplot(data = filtered_years, aes(x = year, y = total_volume, group = term ,color = term)) + 
geom_line() + xlim(1850, 2012) + labs(x = "Year", y = "Raw Mentions") + scale_y_continuous(labels = comma)
```

## Your written answer

I didn't need to reevaluate the graphs since the graphs match up. I expect the highest volume percentage to have the 
highest volume which the plots show
# Part F
...
```{r compute-peaks}
year_total_counts_max <- year_total_counts %>% group_by(term) %>% filter(volume_perc == max(volume_perc))
```
...
years_combined <- left_join(year_total_counts, year_total_counts_max, by = 'term') 
years_half_life <- years_combined %>% filter(year.x > year.y,  volume_perc.x < (volume_perc.y/2)) %>% 
group_by(term) %>% filter(year.x == min(year.x)) %>% mutate(half_life = year.x - year.y)
```
...
```{r plot-half-lifes}
ggplot(years_half_life, aes(x = term, y = half_life)) + xlim(1850, 2012) + geom_point() + geom_smooth()
